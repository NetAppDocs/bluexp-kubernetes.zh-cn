...
BlueXP连接器:: Connector 必须使用所需权限在 Azure 中运行。  a Connector,在下方了解更多信息。
网络连接:: Kubernetes 集群和 Connector 之间以及 Kubernetes 集群和 Cloud Volumes ONTAP 之间需要网络连接。  networking requirements,在下方了解更多信息。
RBAC 授权:: 无论是否使用Active Directory、BlueXP都支持启用了RBAC的集群。必须在每个Azure集群上授权BlueXP Connector角色。  up RBAC authorization,在下方了解更多信息。
