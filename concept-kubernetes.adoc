...
使用 link:https://docs.netapp.com/us-en/cloud-manager-setup-admin/index.html["BlueXP"^] (Cloud Manager)以及使用三项功能操作符部署的Asta三项功能的兼容版本、您可以：*添加和管理Kubnetes集群* link:./task/task-k8s-manage-trident.html["安装、升级或卸载Astra Trident"]* link:./task/task-k8s-manage-storage-classes.html["添加和删除存储类"]* link:./task/task-k8s-manage-persistent-volumes.html["查看永久性卷"]* link:./task/task-k8s-manage-remove-cluster.html["删除Kubernetes集群"] 从工作空间* link:./task/task-kubernetes-enable-services.html["激活或查看BlueXP备份和恢复"]
